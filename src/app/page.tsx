...
// Cache duration: 5 minutes (but sessionStorage clears on tab close)
...
function HomeContent() {
  ...
  // Posts state with caching - initialize with cached data
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (storedGuestMode === 'true' || isGuestMode) {
        ...
        // In guest mode, show the feed but without user-specific features
        setIsAuthenticated(true); // Show the feed
        setIsGuest(true); // Mark as guest
        
        // Seed database with mock data for guest (fire and forget)
      ...
      if (cachedUser?.id) {
        ...
        // Still seed and verify in background
        fetch('/api/seed').catch(console.error);
        return;
      }

      try {
        // Seed database with mock data (fire and forget)
        fetch('/api/seed').catch(console.error);
  ...
  }, [currentUserId, isAuthenticated, hasViewedNotifications, hasViewedMessages]);
  ...
  const handleLike = async (postId: string) => {
    ...
    const updatedPosts = prev => prev.map(p => 
      p.id === postId 
  ...
  const handleRepostCallback = (postId: string, isReposted: boolean, repostedBy?: { id: string; handle: string; displayName?: string | null; avatar?: string | null }) => {
    // Update the post with repost status
    setPosts(prev => prev.map(p => {
      if (p.id === postId) {
        return { 
          ...p, 
          ...
          // Add repostedBy info when reposting
          repostedBy: isReposted && repostedBy ? {
            id: repostedBy.id,
            handle: currentUser?.handle || repostedBy.handle,
            displayName: currentUser?.displayName || repostedBy.displayName,
            avatar: currentUser?.avatar || repostedBy.avatar,
          } : isReposted ? {
            id: currentUserId,
          ...
          repostedAt: isReposted ? new Date().toISOString() : null,
        };
      }
      return p;
    }));
  ...
  const showFloatingCompose = currentView !== 'messages';
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main 
        ref={mainContentRef}
        className={`flex-1 container max-w-2xl mx-auto px-4 pb-20 ${currentView !== 'home' ? 'pt-0' : 'pt-3'}`}
      >
        {renderPage()}
      </main>
