...
interface RepostModalProps {
  ...
  onRepost: (postId: string, isReposted: boolean, repostsCount?: number) => void;
...
export function RepostModal({
  ...
  const handleDirectRepost = async () => {
    ...
    try {
      if (post.isReposted) {
        const res = await fetch(`/api/repost?postId=${post.id}&userId=${currentUserId}`, {
          method: 'DELETE',
        });
        const data = await res.json();
        onRepost(post.id, false, data.repostsCount);
        ...
        const data = await res.json();
        onRepost(post.id, true, data.repostsCount);
